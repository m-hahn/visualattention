...
def reformat(mturk_data_file, workers={}, printWorkers=True):
      ...
      for row in mturk_reader:
        if header:
          ...
          if ntrials > 0:
            ...
            #print data_type
            for key in new_column_labels:
              if key == "Answer.doNotRedirect":
                 print "Unknown key "+key
                 continue
              output_row.append(subject_level_data[key])
            output_rows.append(output_row)
    output_rows = map(lambda y: map(lambda x:x.replace("\n", "-NEWLINE_REMOVED-"), y), output_rows) # mhahn, 2018
  ...
  if printWorkers:
    print workers
  return workers # so can call from Python
...
def expire(label):
    os.system("""
        ...
        NAME_OF_EXPERIMENT_FILES=""" + label + """
        label=$HERE/$NAME_OF_EXPERIMENT_FILES
        ./updateHITs.sh -properties $label.properties -success $label.success
        """)
